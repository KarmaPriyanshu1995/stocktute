...
export function drift(count: number, startPrice: number, step: number, startTime = 1_700_000_000, volume = 8_000): Ohlcv[] {
  const rows: Array<[number, number, number, number, number]> = [];
  let price = startPrice;
  for (let i = 0; i < count; i++) {
    const open = price;
    const close = price + step;
    rows.push([open, Math.max(open, close) + 0.4, Math.min(open, close) - 0.4, close, volume]);
    price = close;
  }
